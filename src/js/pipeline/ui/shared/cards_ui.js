...
// Dépend de la structure HTML/CSS actuelle : éviter les changements diffus non cadrés.
    ...
    getAgents().forEach((agent, index) => {
      ...
      <div class="agent-body" id="${p}-body-${agent.id}">
        ...
        <div class="agent-actions">
          ...
          <button class="btn btn-orange" id="${p}-bp-${agent.id}" type="button" data-rule-action="persist" data-agent-id="${agent.id}" disabled>📌 Toujours</button>
          <button class="btn btn-muted" type="button" data-ui-action="open-prompt-lightbox" data-action-arg="${agent.id}">⚙️</button>
          <button class="btn agent-stop-btn" id="${p}-bstop-${agent.id}" type="button" data-pipeline-action="stop-agent" data-pipeline-prefix="${p}" data-pipeline-agent="${agent.id}">⏹</button>
          ...
          ${agent.id === 'tags' ? `<button class="btn btn-orange" id="${p}-bexplore-tags" type="button" data-selection-action="run-tag-explorer" disabled>🔭 Explorer</button>` : ''}
          ${agent.id === 'titre' ? `<button class="btn btn-orange" id="${p}-bexplore-titre" type="button" data-selection-action="run-titre-explorer" disabled>🔭 Explorer</button>` : ''}
        </div>
  ...
  function buildSelectionHTML(agent, p) {
    if (agent.id === 'tags') {
      return `
    <div class="sel-zone sel-zone-hidden tags-selection-zone" id="${p}-sel-tags">
    ...
    <div class="sel-zone" id="${p}-sel-${agent.id}">
      ...
      <div class="titre-manual-shell">
        <div class="titre-manual-head">
          ...
          <span id="${p}-titre-counter-${agent.id}" class="titre-manual-counter">0 / 140</span>
        </div>
        <div class="manual-titre-row">
          <input type="text" id="${p}-titre-manual-${agent.id}" placeholder="Titre personnalisé..." maxlength="140" data-selection-input="titre-counter" data-agent-id="${agent.id}"/>
