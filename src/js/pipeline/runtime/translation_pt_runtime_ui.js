...
(function initPipelineUITranslationPtRuntime(global) {
  global.PipelineUI = global.PipelineUI || {};

  const TRANSLATION_MAPPING_AGENT_ID = 'traduction_pt';
  const TRANSLATION_LISTING_AGENT_ID = 'traduction_listing_pt';
  const TRANSLATION_MAPPING_PROMPT_PATH = 'prompts/traduction/traduction_pt.md';
  const TRANSLATION_LISTING_PROMPT_PATH = 'prompts/traduction/traduction_listing_pt.md';
  const STORAGE_KEY_PREFIX = 'pipeline.translation.pt.';
  const PREFIXES = ['tt', 'col'];
  const TITLE_MAX_LENGTH = 140;
  const TAG_MAX_LENGTH = 30;
  const DEFAULT_MAPPING_OUTPUT = '— pas encore vérifié —';
  const DEFAULT_MAPPING_STATUS = 'En attente d’un check FR -> PT.';
  const DEFAULT_SOURCE_STATUS = 'En attente d’une fiche Etsy source.';
  const DEFAULT_TRANSLATION_OUTPUT = '— pas encore traduit —';
  const DEFAULT_TRANSLATION_STATUS = 'En attente d’une traduction PT.';
  ...
  const ensurePrefixState = (prefix) => {
    const state = getState();
    state.translationPtByPrefix = state.translationPtByPrefix || {};
    state.translationPtByPrefix[prefix] = state.translationPtByPrefix[prefix] || createEmptyPrefixState();
    const entry = state.translationPtByPrefix[prefix];
  ...
  const loadPrefixState = (prefix) => {
    ...
    try {
      ...
      Object.assign(entry, {
        activeSubtab: parsed.activeSubtab === 'pt' ? 'pt' : 'fr',
  ...
  const extractTranslationFields = (rawText = '') => {
    ...
    if (parsed && typeof parsed === 'object') {
      const tags = Array.isArray(parsed.tags_pt)
        ? parsed.tags_pt
        : Array.isArray(parsed.tagsPt)
          ? parsed.tagsPt
          : Array.isArray(parsed.tags)
            ? parsed.tags
            : typeof parsed.tags_pt === 'string'
              ? parsed.tags_pt.split(',')
              : typeof parsed.tagsPt === 'string'
                ? parsed.tagsPt.split(',')
                : typeof parsed.tags === 'string'
                  ? parsed.tags.split(',')
                  : [];
      return {
        title: String(parsed.title_pt || parsed.titlePt || parsed.title || parsed.title_en || parsed.titleEn || '').trim(),
        description: String(parsed.description_pt || parsed.descriptionPt || parsed.description || parsed.description_en || parsed.descriptionEn || ''),
        tags,
      };
    }

    return {
      title: extractLooseJsonString(rawText, 'title_pt') || extractLooseJsonString(rawText, 'title'),
      description: extractLooseJsonString(rawText, 'description_pt') || extractLooseJsonString(rawText, 'description'),
      tags: extractLooseJsonTags(rawText, 'tags_pt').length
        ? extractLooseJsonTags(rawText, 'tags_pt')
  ...
  const updateMappingStateFromFields = (prefix) => {
    const entry = ensurePrefixState(prefix);
    entry.characterFr = getTrimmedValue(prefix, 'translation-pt-character-fr');
    entry.universeFr = getTrimmedValue(prefix, 'translation-pt-universe-fr');
    entry.characterEn = getTrimmedValue(prefix, 'translation-pt-character-en');
    entry.universeEn = getTrimmedValue(prefix, 'translation-pt-universe-en');
  ...
  const updateListingStateFromFields = (prefix, options = {}) => {
    ...
    listingDraft.translatedTitle = String(readField(prefix, 'translation-pt-translated-title')?.value || '');
    listingDraft.translatedDescription = String(readField(prefix, 'translation-pt-translated-description')?.value || '');
    persistPrefixState(prefix);
  ...
  const renderTagsList = (prefix, kind) => {
    ...
    const tagsHost = readField(prefix, `translation-pt-${kind}-tags-list`);
    const tagInput = readField(prefix, `translation-pt-${kind}-tags-input`);
    ...
    const removeAction = isSource ? 'translation-pt-source-tag-remove' : 'translation-pt-translated-tag-remove';
    const editAction = isSource ? 'translation-pt-source-tag-edit' : 'translation-pt-translated-tag-edit';
    const emptyLabel = isSource ? 'Aucun tag source pour le moment.' : 'Aucun tag PT genere pour le moment.';
    if (!tagsHost) return;
  ...
  const renderPrefixState = (prefix) => {
    ...
    const fieldValues = {
      'translation-pt-character-fr': entry.characterFr,
      'translation-pt-universe-fr': entry.universeFr,
      'translation-pt-character-en': entry.characterEn,
      'translation-pt-universe-en': entry.universeEn,
      'translation-pt-listing-ref': listingDraft.listingRef,
      'translation-pt-source-title': listingDraft.sourceTitle,
      'translation-pt-source-description': listingDraft.sourceDescription,
      'translation-pt-translated-title': listingDraft.translatedTitle,
      'translation-pt-translated-description': listingDraft.translatedDescription,
    };
    ...
    const mappingOutputNode = readField(prefix, 'out-traduction_pt');
    ...
    const mappingStatusNode = readField(prefix, 'translation-pt-status');
    if (mappingStatusNode) mappingStatusNode.textContent = entry.status || DEFAULT_MAPPING_STATUS;

    const sourceStatusNode = readField(prefix, 'translation-pt-source-status');
    if (sourceStatusNode) sourceStatusNode.textContent = listingDraft.sourceStatus || DEFAULT_SOURCE_STATUS;

    const translationOutputNode = readField(prefix, 'out-traduction_listing_pt');
    ...
    const translationDebugNode = readField(prefix, 'translation-pt-debug');
    if (translationDebugNode) {
      translationDebugNode.textContent = String(listingDraft.translationDebug || 'Aucun debug disponible.');
    }

    const translationStatusNode = readField(prefix, 'translation-pt-translation-status');
    if (translationStatusNode) translationStatusNode.textContent = listingDraft.translationStatus || DEFAULT_TRANSLATION_STATUS;

    setMetricNode(
      readField(prefix, 'translation-pt-source-title-meta'),
      `${listingDraft.sourceTitle.length} / ${TITLE_MAX_LENGTH}`,
      listingDraft.sourceTitle.length <= TITLE_MAX_LENGTH,
    );
    setMetricNode(
      readField(prefix, 'translation-pt-translated-title-meta'),
      `${listingDraft.translatedTitle.length} / ${TITLE_MAX_LENGTH}`,
      listingDraft.translatedTitle.length <= TITLE_MAX_LENGTH,
    );

    const sourceTagsMeta = buildTagsMeta(listingDraft.sourceTags);
    setMetricNode(readField(prefix, 'translation-pt-source-tags-meta'), sourceTagsMeta.text, sourceTagsMeta.isValid);

    const translatedTagsMeta = buildTagsMeta(listingDraft.translatedTags);
    setMetricNode(readField(prefix, 'translation-pt-translated-tags-meta'), translatedTagsMeta.text, translatedTagsMeta.isValid);

    renderTagsList(prefix, 'source');
    renderTagsList(prefix, 'translated');
    syncTextareaHeight(readField(prefix, 'translation-pt-source-description'));
    syncTextareaHeight(readField(prefix, 'translation-pt-translated-description'));
  };
  ...
  async function runTranslationPtCheck(prefix = global.pfx()) {
    ...
    const runButton = readField(prefix, 'translation-pt-run');
    ...
    try {
      ...
      const filled = template
        ...
        .replace(/\[\[CHARACTER_PT_CURRENT\]\]/g, entry.characterEn)
        .replace(/\[\[UNIVERSE_PT_CURRENT\]\]/g, entry.universeEn)
      ...
      setMappingStatus(prefix, `Verification PT en cours (${getModel(TRANSLATION_MAPPING_AGENT_ID)})...`);
      ...
      if (parsed && typeof parsed === 'object') {
        entry.characterEn = String(parsed.character_pt || parsed.characterNamePt || parsed.character_en || parsed.characterNameEn || entry.characterEn || '').trim();
        entry.universeEn = String(parsed.universe_pt || parsed.licensePt || parsed.universe_en || parsed.licenseEn || entry.universeEn || '').trim();
      }

      setMappingStatus(prefix, 'Check FR -> PT termine.');
      renderPrefixState(prefix);
      persistPrefixState(prefix);
      global.showToast('Check traduction PT termine');
    } catch (error) {
      entry.output = '';
      setMappingStatus(prefix, `Erreur check PT : ${error.message}`);
      global.showToast(`Erreur traduction PT: ${error.message}`, '#ff4757');
    } finally {
      if (runButton) runButton.disabled = false;
    }
  }

  async function loadTranslationPtSource(prefix = global.pfx()) {
    updateListingStateFromFields(prefix);
    const entry = ensurePrefixState(prefix);
    const listingDraft = entry.listingDraft;
    const loadButton = readField(prefix, 'translation-pt-load-source');
    ...
    try {
      ...
      renderPrefixState(prefix);
      persistPrefixState(prefix);
      global.showToast('Fiche source PT chargee');
    } catch (error) {
  ...
  async function runTranslationPtListing(prefix = global.pfx(), options = {}) {
    updateMappingStateFromFields(prefix);
    updateListingStateFromFields(prefix);
    ...
    const entry = ensurePrefixState(prefix);
    const listingDraft = entry.listingDraft;
    const translateButton = readField(prefix, 'translation-pt-translate');
    ...
    if (!listingDraft.sourceTitle || !listingDraft.sourceDescription || !sourceTagsCsv) {
      ...
      global.PipelineUITranslationEnRuntime?.setTranslationLanguageStatus?.(prefix, 'pt', 'error');
    ...
    global.PipelineUITranslationEnRuntime?.resetTranslationLanguageRunState?.(prefix, 'pt');
    global.PipelineUITranslationEnRuntime?.setTranslationLanguageStatus?.(prefix, 'pt', 'running');
    try {
      ...
      const filled = template
        .replace(/\[\[CHARACTER_FR\]\]/g, entry.characterFr)
        .replace(/\[\[UNIVERSE_FR\]\]/g, entry.universeFr)
        .replace(/\[\[CHARACTER_PT\]\]/g, entry.characterEn)
        .replace(/\[\[UNIVERSE_PT\]\]/g, entry.universeEn)
      ...
      setTranslationStatus(prefix, `Traduction PT en cours (${getModel(TRANSLATION_LISTING_AGENT_ID)})...`);
      ...
      listingDraft.translatedDescription = buildInjectedTranslationDescription(prefix, 'pt', extracted.description || '');
      listingDraft.translatedTags = getEtsyData().normalizeAttributeTags?.(extracted.tags || []) || extracted.tags || [];
      listingDraft.pendingTranslatedTagsInput = '';
      const descriptionField = readField(prefix, 'translation-pt-translated-description');
      if (descriptionField) {
      ...
      setTranslationStatus(prefix, 'Traduction PT terminee.');
      renderPrefixState(prefix);
      persistPrefixState(prefix);
      global.PipelineUITranslationEnRuntime?.setTranslationLanguageStatus?.(prefix, 'pt', 'success');
      if (!silent) global.showToast('Traduction PT terminee');
      global.PipelineUITranslationEnRuntime?.finalizeTranslationCacheRun?.(prefix, 'Traduction PT terminee', ownsCacheRun);
      return { ok: true };
    } catch (error) {
      listingDraft.translationOutput = '';
      listingDraft.translationDebug = '';
      setTranslationStatus(prefix, `Erreur traduction PT : ${error.message}`);
      global.PipelineUITranslationEnRuntime?.setTranslationLanguageStatus?.(prefix, 'pt', 'error');
      if (!silent) global.showToast(`Erreur traduction PT: ${error.message}`, '#ff4757');
      global.PipelineUITranslationEnRuntime?.finalizeTranslationCacheRun?.(prefix, `Erreur traduction PT : ${error.message}`, ownsCacheRun);
  ...
  function copyTranslationPtOutput(prefix = global.pfx()) {
  ...
  function showTranslationPtInput(prefix = global.pfx()) {
    const raw = String(
  ...
  function copyTranslationPtListingOutput(prefix = global.pfx()) {
  ...
  function showTranslationPtListingInput(prefix = global.pfx()) {
    const raw = String(
      ensurePrefixState(prefix).listingDraft.translationInput
    ...
    document.getElementById('rawInputTitle').textContent = '</> INPUT - Traduction fiche PT';
    document.getElementById('rawInputTextarea').value = raw;
    document.getElementById('rawInputCount').textContent = `${raw.length.toLocaleString()} car.`;
    document.getElementById('rawInputLightbox').classList.add('visible');
  }

  function copyTranslationPtField(prefix = global.pfx(), fieldKey = '') {
    const listingDraft = ensurePrefixState(prefix).listingDraft;
    const translatedAll = [
      `TITRE :\n${listingDraft.translatedTitle || ''}`,
  ...
  async function publishTranslationPt(prefix = global.pfx()) {
  ...
  function resolveTranslationPtPromptLightboxSpec(id = '') {
  ...
  function bindPrefixInputs(prefix) {
    [
      'translation-pt-character-fr',
      'translation-pt-universe-fr',
      'translation-pt-character-en',
      'translation-pt-universe-en',
    ].forEach((suffix) => {
      const field = readField(prefix, suffix);
    ...
    [
      'translation-pt-listing-ref',
      'translation-pt-source-title',
      'translation-pt-source-description',
      'translation-pt-translated-title',
      'translation-pt-translated-description',
    ].forEach((suffix) => {
      const field = readField(prefix, suffix);
      ...
      if (suffix === 'translation-pt-source-title' || suffix === 'translation-pt-translated-title') {
        field.addEventListener('input', () => {
          const isSourceTitle = suffix === 'translation-pt-source-title';
          const metricNode = readField(prefix, isSourceTitle ? 'translation-pt-source-title-meta' : 'translation-pt-translated-title-meta');
    ...
    const tagsInput = readField(prefix, 'translation-pt-source-tags-input');
    ...
    const translatedTagsInput = readField(prefix, 'translation-pt-translated-tags-input');
  ...
  function initTranslationPtPanel(prefix) {
    loadPrefixState(prefix);
    bindPrefixInputs(prefix);
    renderPrefixState(prefix);
  }

  function initAllTranslationPtPanels() {
    PREFIXES.forEach((prefix) => initTranslationPtPanel(prefix));
  }

  global.PipelineUITranslationPtRuntime = {
    TRANSLATION_MAPPING_AGENT_ID,
    TRANSLATION_LISTING_AGENT_ID,
    ensureTranslationPtState: ensurePrefixState,
    persistTranslationPtState: persistPrefixState,
    renderTranslationPtState: renderPrefixState,
    initTranslationPtPanel,
    initAllTranslationPtPanels,
    runTranslationPtCheck,
    loadTranslationPtSource,
    runTranslationPtListing,
    copyTranslationPtOutput,
    showTranslationPtInput,
    copyTranslationPtListingOutput,
    copyTranslationPtField,
    publishTranslationPt,
    showTranslationPtListingInput,
    resolveTranslationPtPromptLightboxSpec,
  };

  global.PipelineUI.translationPt = global.PipelineUI.translationPt || {};
  Object.assign(global.PipelineUI.translationPt, global.PipelineUITranslationPtRuntime);
  Object.assign(global, global.PipelineUITranslationPtRuntime);
  ...
  global.resolveCustomPromptLightboxSpec = function resolveCustomPromptLightboxSpec(id = '') {
    return resolveTranslationPtPromptLightboxSpec(id) || previousCustomPromptResolver?.(id) || null;
  };
