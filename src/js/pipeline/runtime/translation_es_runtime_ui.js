...
(function initPipelineUITranslationEsRuntime(global) {
  ...
  async function runTranslationEsListing(prefix = global.pfx(), options = {}) {
    updateMappingStateFromFields(prefix);
    updateListingStateFromFields(prefix);
    ...
    if (!listingDraft.sourceTitle || !listingDraft.sourceDescription || !sourceTagsCsv) {
      ...
      global.PipelineUITranslationEnRuntime?.setTranslationLanguageStatus?.(prefix, 'es', 'error');
    ...
    global.PipelineUITranslationEnRuntime?.resetTranslationLanguageRunState?.(prefix, 'es');
    global.PipelineUITranslationEnRuntime?.setTranslationLanguageStatus?.(prefix, 'es', 'running');
    try {
      ...
      listingDraft.translatedDescription = buildInjectedTranslationDescription(prefix, 'es', extracted.description || '');
      ...
      renderPrefixState(prefix);
      persistPrefixState(prefix);
      global.PipelineUITranslationEnRuntime?.setTranslationLanguageStatus?.(prefix, 'es', 'success');
      if (!silent) global.showToast('Traduction ES terminee');
      global.PipelineUITranslationEnRuntime?.finalizeTranslationCacheRun?.(prefix, 'Traduction ES terminee', ownsCacheRun);
      return { ok: true };
    } catch (error) {
      listingDraft.translationOutput = '';
      ...
      global.PipelineUITranslationEnRuntime?.setTranslationLanguageStatus?.(prefix, 'es', 'error');
      if (!silent) global.showToast(`Erreur traduction ES: ${error.message}`, '#ff4757');
      global.PipelineUITranslationEnRuntime?.finalizeTranslationCacheRun?.(prefix, `Erreur traduction ES : ${error.message}`, ownsCacheRun);
  ...
  async function publishTranslationEs(prefix = global.pfx()) {
  ...
  function bindPrefixInputs(prefix) {
    ...
    [
      ...
      if (suffix === 'translation-es-source-title' || suffix === 'translation-es-translated-title') {
        field.addEventListener('input', () => {
          const isSourceTitle = suffix === 'translation-es-source-title';
          const metricNode = readField(prefix, isSourceTitle ? 'translation-es-source-title-meta' : 'translation-es-translated-title-meta');
