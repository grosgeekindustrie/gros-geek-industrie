(function initPipelineUIEtsyWorkspaceOptionsUi(global) {
  ...
  function renderOptionsStep(prefix, deps = {}) {
    ...
    const stockBlock = `
      <section class="etsy-api-options-group">
        <div class="etsy-api-options-group-head">
          <div>
            <h5>Stock</h5>
            <p>Quantite de base et renouvellement automatique de la fiche.</p>
          </div>
        </div>
        <div class="etsy-api-options-stock-grid">
          <div class="fg">
            <label for="etsyApiOptionsBaseQuantity-${prefix}">Quantite</label>
            <input
              type="number"
              min="0"
              step="1"
              id="etsyApiOptionsBaseQuantity-${prefix}"
              data-js="etsy-options-base-quantity"
              value="${String(draft?.baseQuantity ?? 0)}"
            />
          </div>
          <div class="etsy-api-options-stock-toggle">
            <div>
              <label>Demandes de reapprovisionnement</label>
              <p class="etsy-api-field-hint">Active le renouvellement automatique de la fiche si necessaire.</p>
            </div>
            <label class="etsy-api-options-switch" aria-label="Activer ou desactiver les demandes de reapprovisionnement">
              <input type="checkbox" data-js="etsy-options-auto-renew" ${draft?.autoRenew !== false ? 'checked' : ''}>
    ...
    host.querySelector('[data-js="etsy-options-base-quantity"]')?.addEventListener('input', (event) => {
      const nextQuantity = Math.max(0, Number.parseInt(String(event.target.value || '0'), 10) || 0);
      deps.updateOptionsDraft?.(prefix, (nextDraft) => {
        nextDraft.baseQuantity = nextQuantity;
        (nextDraft.products || []).forEach((product) => {
          product.quantity = nextQuantity;
        });
      });
    });

    host.querySelector('[data-js="etsy-options-auto-renew"]')?.addEventListener('change', (event) => {
      deps.updateOptionsDraft?.(prefix, (nextDraft) => {
        nextDraft.autoRenew = !!event.target.checked;
      });
    });
