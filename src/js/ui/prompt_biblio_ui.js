...
// VALIDÉS / BLACKLISTÉS. À garder cohérent avec la structure des prompts disque.
  ...
  function buildPrompt(agentId, ctx) {
    ...
    const filled = template
      ...
      .replace(/\[\[TARGET_LANGUAGE\]\]/g, ctx.targetLanguage || '')
      .replace(/\[\[TARGET_LANGUAGE_LABEL\]\]/g, ctx.targetLanguageLabel || '')
      .replace(/\[\[ACTIVE_LANGUAGES\]\]/g, ctx.activeLanguages || '')
      .replace(/\[\[SOURCE_TITLE\]\]/g, ctx.sourceTitle || '')
      .replace(/\[\[SOURCE_TAGS\]\]/g, ctx.sourceTags || '')
      .replace(/\[\[SOURCE_DESCRIPTION\]\]/g, ctx.sourceDescription || '')
      .replace(/\[\[SOURCE_ALT\]\]/g, ctx.sourceAlt || '')
      .replace(/\[\[SOURCE_NAME\]\]/g, ctx.sourceName || '')
      .replace(/\[\[SOURCE_UNIVERS\]\]/g, ctx.sourceUniverse || '')
      .replace(/\[\[KNOWN_ALIASES\]\]/g, ctx.knownAliases || '')
      .replace(/\[\[TARGET_ALIAS_NAME\]\]/g, ctx.targetAliasName || '')
      .replace(/\[\[TARGET_ALIAS_UNIVERS\]\]/g, ctx.targetAliasUniverse || '')
      .replace(/\[\[TRANSLATION_FORMAT\]\]/g, ctx.translationFormat || '')
      + (ctx.rules ? `\nRègles permanentes:\n${ctx.rules}` : '')
