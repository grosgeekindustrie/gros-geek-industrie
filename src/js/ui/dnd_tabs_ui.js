...
// Cette couche garde le flow Tabletop dans view-form avec un seul panneau courant.
  ...
  const getRoot = () => document.querySelector('[data-js="dnd-solo-tabs"]');
  ...
  function hasPipelinePanel() {
    return isVisible(document.getElementById('pipeline-tt'));
  }

  function hasResultPanel() {
    return isVisible(document.getElementById('finalOutput-tt'));
  }

  function hasSocialOutput() {
    return isVisible(document.getElementById('socialOutput-tt'));
  }
  ...
  function activateDndSoloTab(tabId, options = {}) {
  ...
  function refreshDndSoloTabs() {
    renderDndSoloTabs();
  }

  function resetDndSoloTabs() {
    activeTabId = 'form';
    renderDndSoloTabs();
  }

  function bindDndSoloTabs() {
    const root = getRoot();
    if (!root || isBound) return;

    root.addEventListener('click', (event) => {
      const button = event.target.closest('[data-dnd-tab]');
      if (!button) return;
      activateDndSoloTab(button.dataset.dndTab);
    });

    ['tt-fNomCourt', 'tt-fNom'].forEach((fieldId) => {
      const field = document.getElementById(fieldId);
      field?.addEventListener('input', refreshDndSoloTabs);
    });

    isBound = true;
  }

  function initDndSoloTabs() {
    if (!getRoot()) return;
    bindDndSoloTabs();
    renderDndSoloTabs();
  }

  function isDndSoloResultAvailable() {
    return hasResultPanel();
  }
