...
Génère exactement 5 titres Etsy en français.
...
Pense comme un rédacteur produit Etsy, pas comme un générateur de tags.

Avant d’écrire, comprends la pièce : son personnage, son univers, son type de pièce, ses archétypes, son medium et les pistes SEO utiles. Ces données servent à nourrir ta réflexion, pas à être vidées mécaniquement dans le titre.
...
Le titre doit toujours rester centré sur l’objet vendu.

Les informations techniques décrivent la pièce physique, pas le personnage, l’univers ou le lore.

Les expressions comme `en résine`, `imprimé en 3D`, `imprimée en 3D` et `à peindre` doivent qualifier clairement un objet concret : une figurine, une statue ou un garage kit.

Chaque titre doit contenir l’expression `à peindre`, attachée clairement à l’objet vendu.

N’écris pas comme si le personnage, l’univers ou le lore étaient eux-mêmes en résine, imprimés en 3D ou à peindre.
...
Les deux souffles doivent se compléter naturellement. Le premier ne doit pas être une simple amorce, et le second ne doit pas devenir un fourre-tout de mots-clés restants.
...
Chaque titre doit contenir au moins une expression forte de positionnement boutique :

* `figurine de collection`
* `statue de collection`
* `garage kit de collection`
* `figurine de collection en résine`
* `statue de collection en résine`
* `garage kit de collection en résine`

Ces expressions sont importantes pour le SEO Etsy et pour expliquer immédiatement le type de produit vendu.

Chaque titre doit aussi utiliser deux termes produit différents parmi :

* `figurine`
* `statue`
* `garage kit`

Ces deux termes doivent être intégrés naturellement, sans empilement. N’utilise pas les trois par réflexe.

Les deux termes produit doivent vraiment servir le titre. Évite les titres où un seul terme produit porte toute la fiche pendant que l’autre partie du titre ne contient plus qu’une cible, un archétype ou un angle SEO. Dans un bon titre, les deux souffles participent à faire comprendre le produit vendu.

## Type de pièce

Respecte le type de pièce fourni dans `[[TYPE]]`.

N’utilise pas un format spécialisé absent des données, comme `miniature`, `buste`, `chibi` ou `tabletop`.

Les termes généraux de la boutique, comme `figurine`, `statue` et `garage kit`, peuvent être utilisés quand ils restent cohérents avec le produit vendu.
...
Utilise ces données seulement si elles rendent le titre plus clair, plus naturel, plus attractif ou plus utile pour Etsy. Ne place jamais un mot simplement parce qu’il est présent dans les données.
...
La fin du titre sert à conclure proprement l’intention d’achat.

Dans le style de la boutique, la fin attendue est souvent une formule du type `pour collectionneurs et fans de...`.

Utilise cette formule dès qu’elle rentre naturellement dans la limite de 140 caractères.

Le complément après `fans de` doit être clair et utile : un univers, un medium, un genre, un archétype ou une recherche Etsy pertinente.

Si la formule complète ne rentre pas proprement, raccourcis avec une fin du type `pour fans de...`.
...
Utilise les expressions métier proprement.

Les formulations comme `en résine`, `imprimé en 3D`, `imprimée en 3D` et `à peindre` doivent rester attachées à l’objet vendu.

Ne casse jamais ces expressions pour gagner de la place.

Écris `en résine imprimée en 3D` ou `en résine imprimé en 3D`.

N’écris jamais `résine 3D`, `imprimé 3D résine`, `imprimée 3D résine`, `imprimé en 3D résine` ou `imprimée en 3D résine`.

## À éviter

N’utilise jamais de pipe `|`.

N’utilise jamais le mot `personnage` dans les titres.

N’utilise pas les échelles dans les titres.

N’utilise pas de slogan publicitaire.

N’écris pas un titre en blocs SEO.

N’écris pas une fin de titre qui ressemble à une liste de tags.
...
Ne fais jamais croire que le produit est peint.
